/*
Name: ngSocial
Directives: ngProfile, ngShare
Version: 1.0
Plugin URI: https://github.com/DannyMcwaves/social
Description: To share any page and list social media profiles with 46 icons 
Author: Danny Mcwaves
Author URI: http://dannymcwaves.herokuapp.com
*/

/* DIRECTIVE USAGE */
/* 

button(ng-share="twitter,facebook,google,linkedin,pinterest,myspace,stumbleupon" animation="") SHARE

button(ng-profile={facebook: 'danny_mcwaves', twitter:'DannyMcwaves', google:'DannyMcwaves'} animation="chain") CONNECT
...
(function($){
  ...
  app.directive("ngProfile", function () {
    ...
    return {
        ...
        link: function (scope, element, attr) {
            var profileObject = JSON.parse(attr.ngProfile);
            profileObject.animation = attr.animation || "launchpadReverse";
            profileObject.blur = attr.blur;
            window.$(element).socialProfiles(profileObject);
        }
    };
  });
  ...
  app.directive("ngShare", function () {
    ...
    return {
        restrict: "A",
        link: function (scope, element, attr) {
            window.$(element).socialShare({
                social: attr.ngShare,
                animation: attr.animation || "launchpadReverse",
                blur: attr.blur
            });
        }
    };
